function booklog(obj) {
    ...
    var image_path = '//widget.booklog.jp/blogparts/images/templates/' + template + '/';
    var image_top = image_path + 'top.gif';
    var image_main = image_path + 'main.gif';
    var image_bottom = image_path + 'bottom.gif';

    var no_image = '//widget.booklog.jp/blogparts/images/common/noimage.gif';
    ...
    html += '<div style="margin:0 auto;padding:0;width:200px;height:14px;background-image:url(' + image_top + ');background-repeat:no-repeat;background-position:right bottom;">';
    html += '</div>';
    ...
    for (var i = 0; i < books.length; i++) {
        ...
        if (i % 5 == 0) {
            html += '<div style="margin:0 auto;padding:0;text-align:left;width:200px;height:64px;background-image:url(' + image_main + ');">';
        }
    ...
    html += '<div style="margin:0 auto;padding:0;text-align:left;width:200px;height:24px;background-image:url(' + image_bottom + ');background-repeat:no-repeat;background-position:right top;">';
    html += '<span style="position:relative;top:2px;left:13px;">';
    if (category.id) {
        var title = tana.name + ' - ' + category.name;
        html += '<a href="//booklog.jp/users/' + tana.account + '?category_id=' + category.id + '" target="_blank" title="' + title + '" style="position:static;display:inline;background:none;margin:0;padding:0;border:0;text-decoration:none;color:#000;">';
        ...
        var title = tana.name;
        html += '<a href="//booklog.jp/users/' + tana.account + '" target="_blank" title="' + title + '" style="position:static;display:inline;background:none;margin:0;padding:0;border:0;text-decoration:none;color:#000;">';
        html += '<img src="' + home + '"  border="0" width="13" height="16" alt="' + title + '" style="position:static;background:none;border-width:0;margin:0;padding:0;display:inline;width:13px;height:16px;" />';
        html += '</a>';
    }
    html += '</span>';
    ...
    html += '<a href="//booklog.jp/" target="_blank" title="booklog.jp" style="position:static;display:inline;background:none;margin:0;padding:0;border:0;text-decoration:none;color:#000;">';
    ...
    div.innerHTML = html;
    document.getElementById('booklog_minishelf').parentNode.appendChild(div);
}